...
We used the CLICS3 database (Rzymski et al., 2020) to obtain an estimate of the proportion of colexifications in common between two languages. A colexification is when the same word is used (within a language) to refer to two concepts. For a given concept $X$ in language $A$, we can look up the concepts $Y_A$ that are colexified with it. We can also do this for concept $X$ in language $B$ to get colexified concepts $Y_B$. We can then calculate the proportion of overlap between the two sets of colexifications. This measures the amount of overlap in the `network' of colexifications between the two languages for a single concept. For the analyses below, we then calculate the average colexification measure for each language pair. 

The colexification data is available in `../data/FAIR/CLICS_by_concept.csv`. Colexification data was not available for 5% of language pairs. 
...
```{r}
colex = read.csv("../data/FAIR/CLICS_by_concept.csv",
                 stringsAsFactors = F)
colex <- colex %>% mutate(langPair = paste(
  pmin(l1,l2),pmax(l1,l2),sep="_"))

colex <- colex %>% 
    group_by(langPair) %>%
    summarize(overlap=mean(overlap,na.rm=TRUE),
              l1_iso2 = first(l1),l2_iso2 = first(l2))

ling <- ling %>% mutate(langPair = paste(
  pmin(l1,l2),pmax(l1,l2),sep="_"))

ling$overlap = colex[match(ling$langPair,colex$langPair),]$overlap
ling$colex = scale(ling$overlap)
```
...
Below we show three things: 

Firstly, the average colexification overlap between two languages predicts semantic alignment. Secondly, the relationship between semantic alignment and cultural similarity remains significant when controlling for colexification. Thirdly, the colexification variable does not predict cultural distance, and only weakly predicts geographic distance.

The first set of models predict the distributional semantics measure by the cultural similarity measure. We show that this is robust to adding the colexification measure:

```{r}
# baseline model with just comparison count
m0Colex = lmer(
  rho.center ~ 1 + comparison_count.center +
    (1 + cult.sim.center  || family.group) +
    (1 + cult.sim.center  || area.group),
...
We also show that the colexification measure does not predict cultural similarity:

```{r}
m0Cult = lmer(cult.sim.center ~ 1 +
  +(1  | family.group) +(1  | area.group),
  data = ling[!is.na(ling$colex),])
m1Cult_by_rho = update(m0Cult,~.+rho.center)
m2Cult_by_colex = update(m1Cult_by_rho,~.+colex)
anova(m0Cult,m1Cult_by_rho,m2Cult_by_colex)
summary(m2Cult_by_colex)
```

Colexification does predict geographic distance, though it has a weaker effect than our distributional measure:

```{r}
m0Geo = lmer(geoDist.center ~ 1 +
  +(1  | family.group) +(1  | area.group),
  data = ling[!is.na(ling$colex),])
m1Geo_by_rho = update(m0Geo,~.+rho.center)
m2Geo_by_colex = update(m1Geo_by_rho,~.+colex)
anova(m0Geo,m1Geo_by_rho,m2Geo_by_colex)
summary(m2Geo_by_colex)
```
...
grphC <- graph.data.frame(colex[,
          c("l1_iso2", 'l2_iso2', 'overlap')], 
          directed=FALSE)
# add value as a weight attribute
colex.m = get.adjacency(grphC, attr="overlap", sparse=FALSE)
...
colex.m[colex.m==0] = mean(colex$overlap)
# Flip to distance
colex.m = 0-colex.m
diag(colex.m) = 0
```
...
Correlation between semantic alignment and colexificaiton:
