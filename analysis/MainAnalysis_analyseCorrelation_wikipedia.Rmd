...
colex$overlap = scale(colex$overlap)

ling <- ling %>% mutate(langPair = paste(
  pmin(l1,l2),pmax(l1,l2),sep="_"))

ling$colex = colex[match(ling$langPair,colex$langPair),]$overlap
ling$colex = scale(ling$colex)
```
...
```{r}
m0Cult_by_Colex = lmer(cult.sim.center ~ 1 + rho.center +
  +(1  | family.group) +(1  | area.group),
  data = ling[!is.na(ling$colex),])
m1Cult_by_Colex = update(m0Cult_by_Colex,~.+colex)
anova(m0Cult_by_Colex,m1Cult_by_Colex)
```

```{r}
m0Geo_by_Colex = lmer(geoDist.center ~ 1 + rho.center +
  +(1  | family.group) +(1  | area.group),
  data = ling[!is.na(ling$colex),])
m1Geo_by_Colex = update(m0Geo_by_Colex,~.+colex)
anova(m0Geo_by_Colex,m1Geo_by_Colex)
```


```{r}
mxx = lmer(geoDist.center ~ 1 + rho.center + colex
  +(1  | family.group) ,
  data = ling[!is.na(ling$colex),])
summary(mxx)
```
