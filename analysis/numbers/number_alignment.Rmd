...
We can look at the importance of the different ways that numeral systems are compased. We use data from Calude & Verkerk (2016), and identify whether two languages have the same system for forming a particular numeral. Calude & Verkerk call this the 'number line'.
...
Danish seems to have a lower average.  We note that some Danish 'crowns' are irregular (50,60,70,80,90).  These pick out many outliers (dots are danish, blue dots are irregualr, violin plots are the rest of the data):
...
-  Being an Uralic language is nota strong predictor.
...
-  Alignment is lower for comparisons between words were at least one has a homophone (`r mp("homophoneTRUE")`)
